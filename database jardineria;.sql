...
/*1*/
Select max(precio_venta) as “precio maximo”, min(precio_venta) as “precio minimo” 
from producto;
...
/*5*/
select nombre as representante, count(*) as clientes
from cliente
join empleado on codigo_empleado = codigo_empleado_rep_ventas
group by codigo_empleado;

/*6*/
select count(*) as "sin presentante"
from cliente
where codigo_empleado_rep_vetas is null;

/*7*/
select min(p.fecha_pago) as "primer pago", max(p.fecha_pago) as "ultima fecha", c.nombre_cliente, p.codigo_cliente
from pago as P
inner join cliente as c on p.codigo_cliente = c.codigo_cliente
group by p.codigo_cliente;

/*8*/
select count(dp.codigo_producto), p.nombre, dp.codigo_pedido
...
/*9*/
Select sum(cantidad), dp.codigo_pedido
from detalle_pedido as dp
join producto as p
where p.codigo_producto = dp.codigo_producto
group by dp.codigo_pedido;
...
/*10*/
select dp.codigo_producto, sum(cantidad) as Total
from detalle_pedido as dp
...
SELECT pro.nombre,
       ...
       SUM(dp.cantidad * dp.precio_unidad) * 1.21 AS total_facturado_iva
FROM detalle_pedido dp
...
/*15*/
 ...
 from cliente where limite_credito in(select Max(limite_credito) 
 from cliente where 1);
...
/*17*/
select pro.nombre, Max(Total_Cantidad) as Mas_vendido
from (
    select dp.codigo_producto, Sum(cantidad) as Total_Cantidad
    from detalle_pedido as dp
    group by dp.codigo_producto
) as maximo
join producto as pro on maximo.codigo_producto = pro.codigo_producto;

/*18*/
Select c.codigo_cliente from cliente as c, pago as p where c.limite_credito > (select sum(p.total)
from pago as p, cliente as c where c.codigo_cliente = p.codigo_cliente)
group by c.codigo_cliente;

/*19*/
select p.nombre, Max(cantidad_en_stock)
from producto as p;

/*20*/
select p.nombre, Min(cantidad_en_stock)
from producto as p;
